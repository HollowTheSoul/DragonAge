def setDragonData():
    dragon = dict()
    #dragon = (name, element, baseAttack, hp, upgrade, range, attack speed)
    dragon[1] = ("FireDragon", "fire", 10, 20, 1, 50, 5)
    dragon[2] = ("FireDragon2", "fire", 10, 20, 2, 100, 4)
    dragon[3] = ("FireDragon3", "fire", 10, 20, 3, 150, 3)

    dragon[4] = ("WaterDragon", "water", 10, 20, 1, 100, 10)
    dragon[5] = ("WaterDragon2", "water", 10, 20, 2, 150, 9)
    dragon[6] = ("WaterDragon3", "water", 10, 20, 3, 1750, 7)

    dragon[7] = ("PoisonDragon", "poison", 7, 20, 1, 75, 8)
    dragon[8] = ("PoisonDragon2", "poison", 7, 20, 2, 125, 6)
    dragon[9] = ("PoisonDragon3", "poison", 7, 20, 3, 150, 5)
    
    dragon[10] = ("EnemyDragon", "normal", 1, 20, 1, 0, 0)
    dragon[11] = ("EnemyDragon2", "normal", 1, 25, 1, 0, 0)
    dragon[12] = ("EnemyDragon3", "normal", 1, 20, 1, 0, 0)
    dragon[13] = ("EnemyDragon4", "normal", 1, 25, 1, 0, 0)
    return dragon
